...
USART0_init:
{
	...
	ldi r16,(1<<UCSZ01)|(1<<UCSZ00)				;COMENTAR ESTA LINEA PARA LEER LA CONFIGURACIÓN DE UCSR0C DE LA EEPROM
...
PWM_init:
{
	;modo pwm phase correct no invertido
	ldi r16,(1<<COM0A1)|(0<<COM0A0)|(0<<COM0B1)|(0<<COM0B0)|(0<<WGM01)|(1<<WGM00)
	out TCCR0A,r16
	;contador de timer 0 = 0
	clr r16
	out TCNT0,r16
	;inicializo el pwm en 0
	out OCR0A,r16
	;habilito interrupción por comparación de OCR0A
	ldi r16,(1<<OCIE0A)
	sts TIMSK0,r16
	;uso preescalado de 256
...
PWM_map:
{
	...
	lpm r17, Z			;dependiendo de como haga esto voy a tener más resolución en el primer valor (0-1). toquetear?
	lds r16, VAL_PWM
	inc r16
_pwm_mapping:
	dec r16
